...
bool UDP_Server::isPast(const udp::endpoint &endpoint)
{
    for (auto i : _past_endpoints) {
        if (i == endpoint)
            return true;
    }
    return false;
...
{
    if (!isPast(_remote_endpoint)) {
        _past_endpoints.push_back(_remote_endpoint);
    }
...
void UDP_Server::handleReceive(const boost::system::error_code &error, std::size_t size)
{
    ...
    // i = 2;
    // for (auto j : _past_endpoints) {
    //     while (i) {
    //         _socket.async_send_to(boost::asio::buffer(_recv_buffer), j,
    //                                 boost::bind(&UDP_Server::handleSend, this, message,
    //                                     boost::asio::placeholders::error,
    //                                         boost::asio::placeholders::bytes_transferred));
    //         i--;
